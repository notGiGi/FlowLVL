...
class ActionRecommender:
            ...
            "order-processing-service": {
                ...
                "actions": {
                    "memory_leak_restart": {
                        ...
                        "conditions": {
                            "metrics": {
                                "memory_usage": "> 65",  # Reducido para mayor sensibilidad
                                "gc_collection_time": "> 400"  # Umbral reducido
                            },
                            "anomaly_score": "> 0.6"  # Umbral reducido
                        },
                        "priority": "high"
                    }
                }
    ...
    def check_conditions(self, conditions, metrics):
        ...
        # Verificar anomaly_score si está presente (con tolerancia)
        if 'anomaly_score' in conditions and 'anomaly_score' in metrics:
            condition = conditions['anomaly_score']
            # Extraer valor umbral
            matches = re.match(r'([<>=!]+)\s*(.+)', condition)
            if matches:
                operator, threshold = matches.groups()
                try:
                    threshold_value = float(threshold)
                    # Dar un 10% de tolerancia al umbral
                    tolerance = threshold_value * 0.1
                    if operator == '>':
                        if metrics['anomaly_score'] <= (threshold_value - tolerance):
                            return False
                except ValueError:
                    if not self.check_condition(f"anomaly_score {condition}", metrics):
                        return False
        ...
        # Todas las condiciones cumplidas (o con flexibilidad)
        return True
